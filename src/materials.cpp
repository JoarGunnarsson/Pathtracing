...
double MicrofacetMaterial::G1(const vec3& half_vector, const vec3& normal_vector, const vec3& v,
...
double MicrofacetMaterial::G(const vec3& half_vector, const vec3& normal_vector, const vec3& incident_vector,
                             const vec3& outgoing_vector, const double alpha) const {
    return G1(half_vector, normal_vector, -incident_vector, alpha) *
           G1(half_vector, normal_vector, outgoing_vector, alpha);
}
...
double MicrofacetMaterial::specular_pdf(const vec3& outgoing_vector, const vec3& incident_vector,
...
vec3 GlossyMaterial::eval(const Hit& hit, const vec3& outgoing_vector, const double u, const double v) const {
    ...
    vec3 diffuse = albedo_map->get(u, v) * 28.0 / (23.0 * M_PI) * (1 - R_0) * (1 - fac1 * fac1 * fac1 * fac1 * fac1) *
                   (1 - fac2 * fac2 * fac2 * fac2 * fac2);
...
vec3 GlossyMaterial::sample_outgoing(const vec3& incident_vector, const vec3& normal_vector, const double u,
...
BrdfData GlossyMaterial::sample(const Hit& hit, const double u, const double v) const {
    ...
    brdf_data.brdf_over_pdf = brdf_data.pdf == 0 ?
                                  0 :
...
double GlossyMaterial::brdf_pdf(const vec3& outgoing_vector, const vec3& incident_vector, const vec3& normal_vector,
                                const double u, const double v) const {
    return 0.5 * (diffuse_pdf(outgoing_vector, normal_vector) +
...
vec3 TransparentMicrofacetMaterial::sample_outgoing(vec3& half_vector, const vec3& incident_vector,
                                                    const vec3& normal_vector, const bool outside, const double u,
                                                    const double v) const {
    double n1, n2;
    if (outside) {
        n1 = external_medium ? external_medium->refractive_index : 1.0;
...
BrdfData TransparentMicrofacetMaterial::sample(const Hit& hit, const double u, const double v) const {
    ...
    double cosine_factor =
        ...
        (dot_vectors(hit.incident_vector, hit.normal_vector) * dot_vectors(half_vector, hit.normal_vector));
